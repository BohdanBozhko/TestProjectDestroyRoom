...
public class Reporter : MonoBehaviour
{
    ...
    {
		GUILayout.BeginArea(new Rect(Screen.width - size.x * 2f, 0f, size.x * 2, size.y * 2));
		if (showLogButton && !show && GUILayout.Button(new GUIContent(images.consoleImage), GUILayout.Width(size.x * 2), GUILayout.Height(size.y * 2)))
		{
			doShow();
		}
		GUILayout.EndArea();
